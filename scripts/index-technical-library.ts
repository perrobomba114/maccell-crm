...
async function cycle(client: pg.PoolClient, pool: pg.Pool, signal: AbortSignal) {
  ...
  if (!acquired) {
    if (!process.argv.includes('--watch')) throw new Error('INDEX_BUSY: otro worker está indexando la biblioteca; reintentá cuando termine');
    return;
  }
  ...
  try {
    ...
    const outcomes = await runBounded(assets, concurrency, async (asset) => {
      if (asset.status !== 'ready') { summary.unsupported++; return; }
      const client = await pool.connect();
      return withIndexConnection(client, signal, async jobSignal => {
      try {
        ...
        if (current && (!job || job.status === 'indexed') && !process.argv.includes('--force')) { summary.cached++; return; }
        if (job?.status === 'failed' && job.asset_sha256 === asset.sha256 && !process.argv.includes('--retry-failed') && !process.argv.includes('--force')) { summary.skippedFailed++; return; }
        ...
        index ??= await extractTechnicalIndex(asset,file,ocrPages,jobSignal);
        ...
        // A lost global lock no longer authorizes job writes: another worker may already own it.
        if (signal.aborted && !stop.signal.aborted) throw new Error('INDEX_CONNECTION_LOST');
        const message = error instanceof Error ? error.message.slice(0,500) : 'Error al extraer el archivo';
        const status = jobSignal.aborted || message.startsWith('INDEX_WRITE_CONFLICT:') ? 'pending' : 'failed';
    ...
    for (const outcome of outcomes) if (outcome.status === 'rejected') {
      summary.failed++;
      process.stderr.write('[TECHNICAL INDEX] No se pudo completar o registrar un trabajo; se reintentará en el siguiente ciclo\n');
    }
...
async function main() {
  ...
  const pool = new pg.Pool({connectionString:process.env.DATABASE_URL,max:concurrency + 1,connectionTimeoutMillis:10_000});
  pool.on('error', () => process.stderr.write('[TECHNICAL INDEX] Conexión inactiva interrumpida\n'));
  try {
    do {
      try {
        const client = await pool.connect();
        await withIndexConnection(client, stop.signal, signal => cycle(client,pool,signal));
      }
