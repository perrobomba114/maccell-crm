...
export const printCashShiftClosureTicket = (data: {
    ...
    const logoUrl = branch?.imageUrl || "/logo.jpg";
    const bonusPerEmp = summary ? Math.round((summary.totalSales * (summary.totalSales > 1000000 ? 0.02 : 0.01)) / 500) * 500 : 0;
    ...
    const content = `
        ...
        <div class="row">
            <span>TOTAL PREMIOS (${summary.totalSales > 1000000 ? "2%" : "1%"}):</span>
