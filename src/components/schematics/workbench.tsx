...
export function SchematicsWorkbench({ initial, userId, canEditIdentity }: { initial: CatalogPage; userId: string; canEditIdentity: boolean }) {
  ...
  return <main ref={expandedView.root} className={`sch-app ${expandedView.expanded ? "sch-expanded" : ""} ${ui.inspector ? "" : "sch-hide-inspector"}`}>
    <WorkbenchHeader expanded={expandedView.expanded} onExpand={() => void expandedView.toggle()} plates={initial.counts.pcbe} documents={initial.counts.pdf} model={boardAsset?.model ?? pdf?.model ?? "Biblioteca técnica"} library={ui.library} inspector={ui.inspector} mode={ui.mode} onLibrary={() => updateUi({ library: !ui.library })} onInspector={() => updateUi({ inspector: !ui.inspector })} onMode={mode => updateUi({ mode })} />
    ...
    <div className={`sch-layout ${ui.library ? "" : "sch-no-library"}`}>
      ...
      <div className="sch-workarea">
        ...
        {pdf && boardAsset && <div className="sch-notice">{linked ? "Referencias vinculadas por identidad validada. Comprobá que la revisión de tu equipo coincide." : "Referencias sin sincronizar: un administrador debe validar marca, modelo, código de placa y revisión de ambos archivos en el Inspector."}</div>}
      </div>
