...
type Props = { toolbar?: ReactNode; id: string; page: number; onPage(page: number): void; references: ReadonlySet<string>; selected: string; onReference(reference: string): void; onTextAvailable(available: boolean): void };
export default function PdfReader({ toolbar, id, page, onPage, references, selected, onReference, onTextAvailable }: Props) {
  ...
  useEffect(() => { container.current?.scrollTo(0, 0); }, [id, page]);
  useEffect(() => {
