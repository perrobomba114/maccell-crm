    ...
    return (
        <div className="border-t bg-background p-3">
            ...
            <div className="flex items-end gap-2">
                ...
                <textarea value={text} onChange={(event) => setText(event.target.value)} onKeyDown={handleKeyDown} maxLength={2000} rows={1} placeholder="Mensaje interno…" className="max-h-28 min-h-10 min-w-0 flex-1 resize-none rounded-xl border bg-muted/30 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-500" />
                <button type="button" aria-label="Enviar mensaje" disabled={sending || (!text.trim() && images.length === 0)} onClick={() => void submit()} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-sky-600 text-white transition-colors hover:bg-sky-500 disabled:cursor-not-allowed disabled:opacity-40"><Send className="h-4 w-4" /></button>
            </div>
