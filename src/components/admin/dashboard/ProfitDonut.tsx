...
export function ProfitDonut({ data, total, onCategorySelect, selectedCategory }: ProfitDonutProps) {
    // Sort large segments first
    const sortedData = [...data].sort((a, b) => b.value - a.value);

    // If a category is selected, we might want to "dim" others or highlight one.
    // For now, let's just pass the selection up.
                            ...
                            >
                                <div className="flex justify-between items-end">
                                    <div className="flex items-center gap-3">
                                        <div
                                            className={cn(
                                                "w-3 h-3 rounded-full shadow-[0_0_10px_rgba(0,0,0,0.5)] transition-transform",
                                        ...
                                        <span className={cn(
                                            ...
                                            isSelected ? "text-white scale-105 origin-left" : "text-zinc-300"
                                        )} title={entry.name}>
                                            {entry.name}
                                        </span>
                                    </div>
                                    <div className="text-right flex items-baseline gap-2">
                                        ...
                                        <span className="text-xs font-bold text-zinc-500 w-[36px] text-right">
                                            {Math.round(percent)}%
                                        </span>
                                    </div>
                                </div>
                    ...
                    {sortedData.length > 5 && (
                        <div className="text-center pt-2">
                            <span className="text-xs text-zinc-600 hover:text-zinc-500 cursor-pointer transition-colors">
                                +{sortedData.length - 5} categorías más
                            </span>
                        </div>
                    )}
                </div>
