                ...
                <CardContent className="p-0">
                    <div className="overflow-hidden">
                        ...
                        <div className="sm:hidden flex flex-col divide-y divide-border/60">
                            {loading ? (
                                <div className="h-24 flex items-center justify-center p-4">Cargando...</div>
                            ) : filteredTransfers.length === 0 ? (
                                <div className="h-24 flex items-center justify-center text-muted-foreground p-4 text-sm text-center">
                                    No hay transferencias para los filtros seleccionados.
                                </div>
                            ) : (
                                filteredTransfers.map((transfer) => (
                                    <div key={transfer.id} className="p-4 flex flex-col gap-3 hover:bg-muted/30 transition-colors">
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="flex flex-col gap-1 min-w-0">
                                                ...
                                                <h3 className="font-bold text-sm leading-tight break-words">{transfer.product.name}</h3>
                                            </div>
                                            <Badge variant="outline" className={cn("shrink-0 text-[10px] px-1.5 h-5", statusStyles[transfer.status])}>
                                                {statusLabels[transfer.status]}
                                            </Badge>
                                        </div>

                                        <div className="flex items-center gap-2 py-2 px-3 bg-muted/50 rounded-lg border border-border/50">
                                            <div className="flex-1 min-w-0">
                                                <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground block mb-0.5">Desde</span>
                                                <span className="text-xs font-bold truncate block">{transfer.sourceBranch.name}</span>
                                            </div>
                                            <ArrowLeftRight className="h-3 w-3 text-muted-foreground shrink-0 mx-1" />
                                            <div className="flex-1 min-w-0 text-right">
                                                <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground block mb-0.5">Hacia</span>
                                                <span className="text-xs font-bold truncate block">{transfer.targetBranch.name}</span>
                                            </div>
                                        </div>

                                        <div className="flex items-center justify-between pt-1">
                                            <div className="flex flex-col">
                                                <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Cantidad</span>
                                                <span className="text-sm font-black tabular-nums">{transfer.quantity}</span>
                                            </div>
                                            <div className="flex gap-2">
                                                <Button variant="outline" size="sm" className="h-8 w-8 p-0" onClick={() => handleEditClick(transfer)}>
                                                    <Edit2 className="h-3.5 w-3.5" />
                                                </Button>
                                                <Button variant="outline" size="sm" className="h-8 w-8 p-0 text-rose-600 hover:bg-rose-500/10" onClick={() => setTransferToDelete(transfer)}>
                                        ...
                                        <div className="text-[9px] text-muted-foreground font-medium text-right">
                                            Creada por {transfer.createdBy.name}
                                        </div>
                                    </div>
                                ))
                            )}
